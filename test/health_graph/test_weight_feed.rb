  ...
  context "weight" do 
    setup do
      ...
      stub_request(:get, HealthGraph.endpoint + '/weight'
        ).with(:headers => {'Authorization' => 'Bearer ' + TEST_USER_TOKEN, 'Accept' => HealthGraph.accept_headers[:weight_feed]}
    ...
    should "make request to api" do       
      ...
      assert_requested :get,  HealthGraph.endpoint + '/weight', :headers => {'Authorization' => 'Bearer ' + TEST_USER_TOKEN, 'Accept' => HealthGraph.accept_headers[:weight_feed]}, :times => 1
    end
